...
<#
.SYNOPSIS
    STEP 2 of 2 — Performs VM-to-IP Group Migration using NSX_VM_Groups-Final.csv.

.DESCRIPTION
    This script automates the transition of NSX Security Groups from VM-based 
    membership to IP-based membership using a two-phase approach driven by 
    a single input file.

    1. CREATE PHASE (-CreateMIGIPGroups):
       Reads 'MIGIP_GroupJson' from the CSV to create new helper groups 
       (e.g., "MIG-IP_WebServers") containing static IPv4 addresses.

    2. PATCH PHASE (-PatchVMGroups):
       Reads 'UpdatedRawJson' from the CSV to update original groups with a 
       PathExpression pointing to the new MIG-IP helper groups. Uses a 
       dependency sorter to ensure nested groups are updated in the correct order.

    3. ROLLBACK PHASE (-RollbackVMGroups):
       Reads 'RawJson' from the CSV to restore groups to their original 
       VM-based logic. Only touches groups that currently contain MIG-IP logic.

    FILE RESOLUTION
    ---------------
    A single Windows file browser dialog opens at the start of the script 
    to select 'NSX_VM_Groups-Final.csv'. This file must contain the following 
    headers: Id, RawJson, MIGIP_GroupName, MIGIP_GroupJson, and UpdatedRawJson.

.PARAMETER NSXManager
    FQDN or IP of the destination NSX Manager.

.PARAMETER InputFolder
    Initial directory for the file picker. Default: Current directory.

.PARAMETER CreateMIGIPGroups
    Phase 1: Create the helper groups containing static IPs. Default: $false

.PARAMETER PatchVMGroups
    Phase 2: Update original groups to reference the MIG-IP groups. Default: $false

.PARAMETER RollbackVMGroups
    Safety: Reverts groups to their original VM-based logic. Default: $false

.EXAMPLE
    # Complete Migration (Create helper groups then Patch original groups)
    # This is the standard operational flow for a VM-to-IP transition.
    .\Import-NSX-VMGroupsToIP.ps1 -NSXManager nsx01.local `
        -InputFolder C:\Migration -CreateMIGIPGroups $true -PatchVMGroups $true

.EXAMPLE
    # Dry Run (What-If)
    # Shows how many groups would be created or patched without making changes.
    # Highly recommended to run this before any live execution.
    .\Import-NSX-VMGroupsToIP.ps1 -NSXManager nsx01.local `
        -InputFolder C:\Migration -PatchVMGroups $true -WhatIf

.EXAMPLE
    # Emergency Rollback
    # Detects groups containing 'MIG-IP_' logic and restores their original 
    # VM-based expressions from the 'RawJson' column of the CSV.
...
param(
    ...
    [bool]$CreateMIGIPGroups  = $false,
    [bool]$PatchVMGroups      = $false,
	[bool]$RollbackVMGroups = $false,
    [string]$LogFile   = (Join-Path $InputFolder "Import-NSX-VMGroupsToIP_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"),
...
Write-Log "Import-NSX-VMGroupsToIP.ps1 v$ScriptVersion" INFO
...
if ($CreateMIGIPGroups -or $PatchVMGroups -or $RollbackVMGroups) {
    $Script:CsvPath_FinalGroups = Resolve-CsvFile -Label 'NSX_VM_Groups-Final.csv'
}
...
# ═════════════════════════════════════════════════════════════
# 1. CREATE MIG-IP GROUPS
# ═════════════════════════════════════════════════════════════
function Create-MIGIPGroups {
    Write-Log "━━━ Phase 1: Creating MIG-IP Helper Groups ━━━" INFO
    #$csvPath = $Script:CsvPath_MIGIPGroups
	$csvPath = $Script:CsvPath_FinalGroups
    #$rows    = Read-CsvFile -ResolvedPath $csvPath -Label 'MIG-IP_ Groups'
	$rows    = Read-CsvFile -ResolvedPath $csvPath -Label 'NSX_VM_Groups-Final.csv'
    if (-not $rows) { return }
        
    foreach ($row in $rows) {
        # Only proceed if there is a MIG-IP group defined
        if ([int]$row.IPv4Count -eq 0 -or [string]::IsNullOrWhiteSpace($row.MIGIP_GroupJson)) {
            continue
        }

        $migName = $row.MIGIP_GroupName
        $path    = "/policy/api/v1/infra/domains/$DomainId/groups/$migName"

        # Check if the MIG-IP group already exists to avoid duplication
        if (Test-ObjectExists -Path $path) {
            if (-not (Resolve-Conflict -ObjectType 'Group' -ObjectId $migName)) { $Stats.Skipped++; continue }
        }

        if ($PSCmdlet.ShouldProcess($migName, "Create MIG-IP Group")) {
            $ok = Invoke-NSXPatch -Path $path -JsonBody $row.MIGIP_GroupJson
            if ($ok) { 
                $Stats.MIGIP_Created++
                Write-Log "  ✔ Created: $migName" SUCCESS 
            } else { 
                $Stats.Errors++ 
            }
        }
...
# ═════════════════════════════════════════════════════════════
function Patch-ExistingGroupsWithMIGIP {
    Write-Log "━━━ Phase 2: Patching Original Groups with MIG-IP Logic ━━━" INFO
    ...
    foreach ($row in $sortedRows) {
        ...
        if ($PSCmdlet.ShouldProcess($id, "Patch Group with IP Logic")) {
...
function Restore-OriginalGroups {
    ...
    foreach ($row in $sortedRows) {
        ...
        # 1. Verification: Only rollback if the group actually contains MIG-IP logic
        $liveGroup = Invoke-NSXGet -Path $path
        $isPatched = $false
        if ($liveGroup -and $liveGroup.expression) {
            # Convert expression to string to easily find the MIG-IP prefix
            $exprString = $liveGroup.expression | ConvertTo-Json -Depth 20
            $isPatched  = $exprString -match "MIG-IP_"
        }

        if (-not $isPatched) {
            Write-Log "  Skipping ${id}: No MIG-IP logic detected." INFO
            $Stats.Skipped++
            continue
        }
...
# MAIN
# ═════════════════════════════════════════════════════════════
$anyAction = $CreateMIGIPGroups -or $PatchVMGroups -or $RollbackVMGroups
if (-not $anyAction) {
...
Write-Log "══════════════════════════════════════════════════" INFO
Write-Log " Create MIG-IP_ groups     : $CreateMIGIPGroups" INFO
...
try {
    ...
    if ($CreateMIGIPGroups) { Create-MIGIPGroups   }
	if ($PatchVMGroups) { Patch-ExistingGroupsWithMIGIP   }
    ...
    if ($Stats.WhatIf_MIGIPCount -gt 0 -or $Stats.WhatIf_VMGroupCount -gt 0 -or $Stats.WhatIf_RollbackCount -gt 0) {
        ...
        if ($Stats.WhatIf_MIGIPCount -gt 0)    { Write-Log "  MIG-IP Groups to Create     : $($Stats.WhatIf_MIGIPCount)" INFO }
        ...
        # Live Results logic
        if ($Stats.MIGIP_Created -gt 0)      { Write-Log "  MIG-IP Groups Created       : $($Stats.MIGIP_Created)" SUCCESS }
        ...
        if ($Stats.MIGIP_Created -eq 0 -and $Stats.VMGroupsPatched -eq 0 -and $Stats.GroupsRestored -eq 0 -and $Stats.Errors -eq 0) {
